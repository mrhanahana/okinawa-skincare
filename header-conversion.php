...
<script>
  <!-- Event snippet for 【検索キャンペーン】より送信完了（okinawa-skincare.com） conversion page -->
  gtag('event', 'conversion', {'send_to': 'AW-754298546/TRJgCPCL54sYELLd1ucC'});
  <!-- Event snippet for 【P-MAXキャンペーン】よりお申込み完了（okinawa-skincare.com） conversion page -->
  gtag('event', 'conversion', {'send_to': 'AW-754298546/vJ5RCJDPz44YELLd1ucC'});
</script>
...
function gtag_tel_conversion(url) {
  ...
  fbq('trackCustom', 'tel', { number: url });
      ...
      <div id="header_button" class="clearfix">
        ...
        for ( $i = 1; $i <= 2; $i++ ):
          if ( $options[ 'show_header_button' . $i ] ) {
            ?>
        <div class="button button<?php echo $i; ?>"> <a href="<?php echo esc_url($options['header_button_url'.$i]); ?>"<?php if($options['header_button_target'.$i]) { echo ' target="_blank"'; }; ?>><?php echo esc_html($options['header_button_label'.$i]); ?></a> </div>
        <!-- END .header_button -->
        <?php }; endfor; ?>
      </div>
      <!-- END #header_button -->
...
<div id="header_slider_wrap">
  ...
  if ( $options[ 'header_content_type' ] == 'type2' || $options[ 'header_content_type' ] == 'type3' || $options[ 'header_content_type' ] == 'type4' ) {
    if ( $options[ 'show_header_catch' ] == 1 ) {
      ...
      ?>
  <div class="caption<?php if($options['header_content_type'] == 'type4') { echo ' para_slider_caption'; }; ?>">
    <div class="caption_inner">
      <?php if($catch || $catch_mobile) { ?>
      <h2 class="title rich_font_<?php echo esc_attr($font_type); if($catch_mobile) { echo ' has_mobile_word'; }; ?>" <?php if($catch_mobile) { echo 'data-label="' .  nl2br(esc_html($catch_mobile)) . '"'; }; ?>><span><?php echo nl2br(esc_html($catch)); ?></span></h2>
      <?php }; ?>
      <?php if($sub_title || $sub_title_mobile) { ?>
      <p class="sub_title<?php if($sub_title_mobile) { echo ' has_mobile_word'; }; ?>"<?php if($sub_title_mobile) { echo ' data-label="' . nl2br(esc_html($sub_title_mobile)) . '"'; }; ?>><span><?php echo nl2br(esc_html($sub_title)); ?></span></p>
      <?php }; ?>
      <?php if( ($show_button == 1) && $url ) { ?>
      <a class="button" href="<?php echo esc_url($url); ?>"<?php if($target == 1) { echo ' target="_blank"'; }; ?>><span><?php echo esc_html($button_label); ?></span></a>
      <?php }; ?>
    </div>
  </div>
  <!-- END .caption -->
  ...
  if ( $options[ 'use_header_overlay' ] == 1 ) {
    ?>
  <div class="overlay<?php if($options['use_header_overlay_gd']) { echo ' gradation'; }; ?>"></div>
  <?php
  };
  }; // END caption
  ?>
  <div id="header_slider"<?php if($options['header_content_type'] == 'type4') { echo ' class="header_para_slider" data-slider-time="'.esc_attr($options['header_para_time']).'"'; } ?>>
    <?php
    // para_slider ***********************************************************************************************************************
